...
library(pacman)
p_load(car, forecast, TSstudio, timeSeries, TSA, ggplot2)
...
phi_sec = seq(from = -0.9, to=0.9, 0.05)

# puntero a carpeta
file.path("~", "Procesos_AR_p")
...
# multiples graficos de proceso AR(1)
theta_sec = seq(from = -0.9, to=0.9, 0.05)

# puntero a carpeta

file.path("~","Procesos_MA_q/")

for (i in 1:length(theta_sec)) {
  nombre <- paste("Grafico_Proc_MA(1)","_theta=",theta_sec[i],".png", sep ="")
  
  png(nombre)
  MA_test <- arima.sim(list(order(0,0,0), ma=theta_sec[i]), sd = sqrt(0.5) 
  ...
  pacf(MA_test, lag.max = 18, main="FACP simple", ylab="", xlab="Rezago")
  dev.off()
...
MA3 <- arima.sim(list(order(0,0,3), ar= c(-0.5,0.2,-0.2)), sd= sqrt(5), n=5000)
